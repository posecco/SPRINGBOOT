...
public class WorkMessagesService {
	...
	public WorkMessages findById(Integer id) {
		Optional<WorkMessages> option = workMessagesDao.findById(id); //Optional<>findById(ID id)物件
	...
	public WorkMessages getLastest() {
		return workMessagesDao.findFirstByOrderByAddedDesc(); //找到第一筆資料根據Added屬性也就是日期，排列方式是Desc降冪排序，從上由大到小
	}
}
